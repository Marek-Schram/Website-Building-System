...
- Build phases: Phase 1 covered the website-client funnel end to end. Phase 2 (done) extends the same board
  to lodging listings — presence detection, a real proposal/listing-kit config flow, and the add-ons section
  correctly staying website-only (it needs a `clients/<slug>` folder lodging deals don't have). Phase 3 is
  polish (marketing-kit, parity checks, real invoicing, follow-up reminders).
